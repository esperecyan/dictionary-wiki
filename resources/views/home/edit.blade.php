        ...
        {!! Form::model(Auth::user(), ['class' => 'form-horizontal']) !!}
            ...
            <div class="form-group">
                <div class="col-md-12">
                            ...
                            @foreach (config('auth.services') as $service)
                                ...
                                <tr>
                                        ...
                                        @if ($externalAccount && $externalAccount->link)
                                            {!! Form::checkbox(
                                                ...
                                                [
                                                    ...
                                                    'title' => $available
                                                        ? null
                                                        : _('アカウントへのリンクを公開するには、連携を許可する必要があります。'),
                                                ]
                                            ) !!}
                                        @endif
                                    </td>
                                    <td>
                                        @if ($externalAccount)
                                            <button
                                                formaction="{{ route('users.external-accounts.update') }}"
                                                name="provider"
                                                value="{{ $service }}">
                                                {{ _('更新') }}
                                            </button>
                                            <button
                                                name="disconnect"
                                                value="{{ $service }}"
                                                {!! $userCanDisconnect ? '' : ' disabled="" title="' . e(_('連携を解除するには、別の外部アカウントを登録する必要があります。')) . '"' !!}>
                                                {{ _('連携を解除') }}
                                            </button>
                                        @else
                                            <button
                                                formaction="{{ route('users.external-accounts.store') }}"
                                                name="provider"
                                                value="{{ $service }}">
                                                {{ _('連携') }}
                                            </button>
                    ...
                    <p>{{ _('※ここで選択するメールアドレスは通知用のものであり、他のユーザーには公開されません。') }}</p>
            ...
            <div class="form-group">
                <div class="col-md-6 col-md-offset-4">
                    {!! Form::button(
                        '<i class="fa fa-btn fa-edit"></i>' . _('更新'),
